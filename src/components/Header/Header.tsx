    ...
    <div className={styles.container}>
      ...
      {popularCurrencies?.map((crypto) => (
        <div className={styles.popular_cryptocurrency} key={crypto.id}>
          <span className={styles.popular_cryptocurrency_title}>{crypto.name}</span>
          <span className={styles.popular_cryptocurrency_price}>{formatLargeNumbers(Number(crypto.priceUsd))} USD</span>
        </div>
      ))}
      </div>
      ...
      {modalOpen && (
        ...
        <div className={styles.modal}  >
          <AiOutlineCloseCircle className={styles.icon_close} onClick={closeModal}/>
